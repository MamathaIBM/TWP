...
@Component({
  selector: 'app-tailored-service-backlog',
  templateUrl: './tailored-service-backlog.component.html',
  styleUrls: ['./tailored-service-backlog.component.css']
})
export class TailoredServiceBacklogComponent implements OnInit,OnDestroy {
    ...
    StatusKeywordsval: any;
    localtwbsval: twbsSerBlog[];
    private updateI : number;
    private ownerIDValidation: number;
    disableSelect = new FormControl(false);
    constructor(private twbsService: TailoredServiceBacklogService,
    ...
    onUpdate() {
        ...
        this.UpdateSubscription = this.twbsService.putTailoredWBSSB(this.listarray).subscribe(res => {
            if (res[0].insert === "success") {
                ...
                //         this.progress = 0;
                //         this.loadData()
                //         this.toastr.success('Inserted Succcessfully', 'Service Backlog')
                //     } else {
                //         this.toastr.error('Insert Fail', 'Service Backlog')
                //     }
                // }, 3000)
            }
            console.log(res)
...
getEmployeeName(obj:any,index) {
    ...
    //    this.nameField.nativeElement.focus();
         this.setFocus('#TServiceBckName'+this.matchID);
    });
    }
